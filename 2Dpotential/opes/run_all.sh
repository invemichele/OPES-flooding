...
# wait for all pids before calculating the time
for pid in ${pids[*]}; do
  wait $pid
done
...
do
  ...
  tempo=`tail -n 1 time | awk '{print $4}'`
  printf "%e\n" $tempo >> ../times.dat
...
rm running_mean-std_times.dat mean-std_times.dat running_mean-std_times_sim.dat mean-std_times_sim.dat
awk '{for(i=1;i<=NR;i++) {sum += $i; sumsq += ($i)^2}};{printf "%e %e \n", sum/NR, sqrt((sumsq-sum^2/NR)/NR)}' times.dat >> running_mean-std_times.dat
awk '{for(i=1;i<=NR;i++) {sum += $i; sumsq += ($i)^2}};{printf "%e %e \n", sum/NR, sqrt((sumsq-sum^2/NR)/NR)}' times_sim.dat >> running_mean-std_times_sim.dat
