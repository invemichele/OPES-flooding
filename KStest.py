...
for i in range(len(fesfiles)) :
    ...
    ax.plot(x1*mu,yfit,'b-',lw=3.,color=colors[i])
    ax.set_xscale('log')
    ax.set_xlabel('Time')
...
plt.xscale('log')
plt.ylim(0,1.005)
plt.title('tau = %0.3e; p = %0.3f'%(float(tau*mu),float(pvalue)),fontsize=16)
plt.tight_layout()
